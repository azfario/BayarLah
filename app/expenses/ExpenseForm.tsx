...
type ManualStep = "DETAILS" | "FRIENDS" | "REVIEW";

const MANUAL_STEPS: { key: ManualStep; label: string }[] = [
...
export default function ExpenseForm({ friends }: ExpenseFormProps) {
  ...
  const [mobileStep, setMobileStep] = useState<ManualStep>("DETAILS");
  ...
  function goToStep(step: ManualStep) {
    setMobileStep(step);
  ...
  function goBack() {
    const currentIndex = MANUAL_STEPS.findIndex((s) => s.key === mobileStep);
    const prev = MANUAL_STEPS[currentIndex - 1];
    if (prev) goToStep(prev.key);
  }

  function goContinue() {
    const currentIndex = MANUAL_STEPS.findIndex((s) => s.key === mobileStep);
    const next = MANUAL_STEPS[currentIndex + 1];
    if (next) goToStep(next.key);
  }

  const isFirstStep = mobileStep === MANUAL_STEPS[0].key;
  const isReview = mobileStep === "REVIEW";
  ...
  return (
    ...
    >
      ...
      <nav
        ref={flowRef}
        aria-label="Expense steps"
        className="scroll-mt-4 rounded-xl border border-[#e5e7eb] bg-white p-2 shadow-sm md:hidden"
      >
        <ol className="grid grid-cols-3 gap-1">
          {MANUAL_STEPS.map((step, index) => {
            const active = step.key === mobileStep;
            return (
              <li key={step.key}>
                <button
                  type="button"
      ...
      {/* ── DETAILS step ──────────────────────────────────────── */}
      <div className={`grid gap-5 md:block ${mobileStep === "DETAILS" ? "" : "hidden"}`}>
      ...
      {/* ── FRIENDS step ──────────────────────────────────────── */}
      <div className={`grid gap-5 md:block ${mobileStep === "FRIENDS" ? "" : "hidden"}`}>
        <section>
        ...
        <section className="rounded-2xl border border-[#e5e7eb] bg-[#f7f8fa] p-4">
      ...
      {/* ── REVIEW step ───────────────────────────────────────── */}
      <div className={`grid gap-5 md:block ${mobileStep === "REVIEW" ? "" : "hidden"}`}>
        {showCustomWarning ? (
      ...
      <div className="hidden md:flex justify-stretch sm:justify-end">
        <SubmitButton
